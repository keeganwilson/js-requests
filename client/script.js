...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/thisisatestparam')
    .then(res => {
...
const myQuery = () => {
    axios.get('http://localhost:3000/query-test?name=keegan&age=39')
    .then(res => {
        console.log(`Query time!${res.data}`);
    })
    .catch((err) => {
        console.log(err)
    })
...
queryButton.addEventListener('click', myQuery)
...
const submitButton = document.querySelector('#foods')
submitButton.addEventListener('submit', createFood)

const clearButton = document.querySelector('#clear')
clearButton.addEventListener('submit', clearList)
